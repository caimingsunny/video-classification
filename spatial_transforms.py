...
# 按顺序将多个transform函数应用于输入图像
class Compose(transforms.Compose):
    
    # 循环遍历每个transform函数并调用它们自己的randomize_parameters函数
...
class CornerCrop(object):
    ...
    def __init__(self,
        ...
        if crop_position is None:
            self.randomize = True # 随机选择一个角落位置进行裁剪
        else:
            self.randomize = False # 将裁剪图像的位置固定在指定的位置
...
# 随机选择一个尺度和角落位置进行裁剪，然后将裁剪后的图像缩放到指定大小
...
class RandomResizedCrop(transforms.RandomResizedCrop):
    ...
    def __init__(self,
                 size,
                 scale=(0.08, 1.0), # 裁剪的尺度范围
                 ratio=(3. / 4., 4. / 3.), # 指定宽高比范围
                 interpolation=Image.BILINEAR): # 指定缩放插值方法
